    ...
    <main className="sidecar" data-motion={motion ? 'on' : 'off'}>
      ...
      {!snapshot ? <section className="empty-state"><CircleHelp size={30} /><h1>{error ? '接続を確認してください' : 'AI-DLCの記録を読み込んでいます'}</h1><p>{error ? 'デモでは、承認待ちやレビュー中の見え方を試せます。' : '工程と直近の動きを確認しています。'}</p><Button variant="outline" onClick={() => changeMode('demo')}>デモを見る</Button></section> : <>
        ...
        <Accordion className="details" multiple>
          ...
          <AccordionItem value="stages"><AccordionTrigger><span className="detail-label"><Layers3 size={16} />全体の工程と担当</span></AccordionTrigger><AccordionContent><ol className="stage-list">{snapshot.stages.map((stage) => <li key={stage.slug} className={stage.slug === snapshot.stage.slug ? 'selected' : ''}><span className="stage-label">{stage.name}<span className="original-name" lang="en">{(stage.englishName ?? stage.slug)}</span></span><small>{stateNames[stage.status]}</small></li>)}</ol></AccordionContent></AccordionItem>
